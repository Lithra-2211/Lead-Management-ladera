...
export const CustomersPage = () => {
    ...
    if (customer) {
      ...
      if (customer.type === 'Dealer') {
        ...
        if (dealerData && dealerData.locations && dealerData.locations.length > 0) {
           ...
           customerLocations = [{ locationName: '', country: 'India', state: '', city: '', addressLine: '', addressLine2: '', landmark: '', pincode: '', contactPerson: '', mobileNumber: '', email: '' }];
        }
  ...
  const onSubmit = async (data: any) => {
    ...
    if (editingCustomer) {
      ...
      const newLocationIds = data.locations?.map((l: any) => l.id).filter(Boolean) || [];
      ...
      for (const loc of data.locations || []) {
        if (loc.id) {
          await updateDealerLocation(loc.id, loc);
        }
      }

      // Add new locations
      const newLocations = data.locations?.filter((l: any) => !l.id) || [];
                      ...
                      <div key={field.id} style={{ marginBottom: '24px', padding: '16px', background: '#F8FAFC', borderRadius: '8px', border: '1px solid #E2E8F0' }}>
                        {locationFields.length > 1 && (
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                            ...
                            <Button type="button" small danger onClick={() => handleDeleteExistingAddress(field.id || '', index)}>Remove Address</Button>
                          </div>
                        )}
